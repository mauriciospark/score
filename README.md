...
SCORE é uma plataforma de auditoria e análise de perfil GitHub que fornece uma avaliação precisa e justa da atividade de desenvolvedores em repositórios públicos. Utilizando um sistema de pontos ponderados, o sistema calcula ranks baseados em contribuições reais, considerando métricas como commits, pull requests, issues, estrelas e contribuições em projetos externos.
...
- Sistema de pontos ponderados para cálculo de ranks
- Algoritmo de ponderação baseado em tipos de contribuição

## Funcionalidades

- **Análise de Perfil**: Busca automática de dados do usuário via API do GitHub
- **Cálculo de Rank**: Sistema de pontos ponderados com escala tradicional (C, C+, B-, B, B+, A-, A, A+, S, S+, S++)
- **Tabela Dinâmica de Metas**:
  - Estado 1: Tabela informativa com todos os ranks e faixas de pontos
...
- Implementa sistema de pontos ponderados para cálculo de ranks
- Sistema de pontos ajustado para priorizar contribuições significativas (contribuições em repositórios externos, estrelas diretas)
